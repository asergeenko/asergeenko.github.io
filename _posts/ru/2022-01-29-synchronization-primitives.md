...
- **барьер** - потоки, дойдя до барьера должны дождаться остальных и только тогда выполняться дальше
- **уведомления** - сообщить, что что-то произошло (инициализация, успешное выполнение задания т.п.)

## Condition

Комбинация блокировки и уведомления. Блокируем критическую секцию и уведомляем об этом остальные потоки.
~~~
...
- ```acquire()```
- ```release()```
- ```wait(timeout=None)``` - вызывает ```release()``` и блокирует поток до вызова другим потоком ```notify()``` или ```notify_all()``` или истечения времени ```timeout```
- ```wait_for(predicate, timeout=None)``` - повторно вызывает wait() до тех пор, пока ```predicate``` не вернёт ```True```
- ```notify(n=1)``` - разблокирует ```n``` ожидающих потоков
- ```notify_all()``` - разблокирует все ожидающие потоки 
