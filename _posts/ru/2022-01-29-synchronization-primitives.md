...
- [Barrier](#barrier)
## Зачем?
![](/images/ru/multiprocessing_beast.jpg)
...
Комбинация захвата и уведомления. При захвате можно уведомить (разблокировать) один или несколько (метод ```notify()```) или все (метод ```notify_all()```) ожидающие потоки. Также используется в архитектуре ```producer-consumer```.
~~~
...
![](/images/ru/multithreaded_barrier.jpg)
~~~
